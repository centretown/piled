...
func (h *Hub) Run() {
	...
	for {
		...
		case <-h.updateList:
			b := []byte("<div id=\"users\" class=\"users\" style=\"display:none;\">" + h.GetClientList() + "</div>")
			h.mutex.Lock()
			for client := range h.clients {
				client.send <- b
			}
...
func (h *Hub) GetClientList() string {
	...
	for client := range h.clients {
		if !client.active {
			buf.WriteString("<div  class=\"user-inactive\">")
			buf.WriteString(client.name)
			buf.WriteString("</i></div>")
		} else {
			buf.WriteString("<div class=\"user-active\">")
			buf.WriteString(client.name)
			buf.WriteString("</b></div>")
		}
	}
	return buf.String()
}
